subroutine NewtonWoSafeg(n,m,x,l,u,scaleF,checkder,outiter,time,nfev,theta,inform)
	...
	! LOCAL SCALARS
	integer :: ind,infoIS,infoISSD,infoLS,infofun,allocerr,maxoutiter,nfevLS,ngev				 
	real(kind=8) :: stp,stpmin,epsopt,sqrtepsopt,ftol,eta,q,thetaSD
	real(kind=8) :: timeini,timefin
	logical :: iprint,iprintLS

	! LOCAL ARRAYS
	real(kind=8) :: g(n),gphi0(m),Fx(m),C(m),lambda(m),dSD(n),lambdaSD(m),dmu(n)
  ...
  !
  ! using the Newton Method without Safeguards described in
  !
  ...
  !
	...
	1001 format(/,1X,'Solver: Newton method without safeguards')
