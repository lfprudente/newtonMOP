subroutine NewtonGradient(n,m,x,l,u,scaleF,checkder,outiter,time,nfev,nhev,chol,theta,inform)
	...
	real(kind=8) :: stp,stpmin,epsopt,ftol,eta,q,mu,multmu,norm2dSD,&
	                norm2d,gamma1,gamma2,minB
  ...
  !
  ! using the Newton Gradient Method with Safeguards described in
  !
  ...
  !
	...
	Main_loop: do
		...
		do	
			...
			if ( infoChol == 0 ) then
				...
				if ( maxval( matmul( JF,d ) ) <= - gamma1 * norm2d * norm2dSD ) exit
		
			end if
			mu = max( multmu * mu, 1.0d0 )
		end do
		
		if ( norm2d < gamma2 * norm2dSD ) d = gamma2 * norm2dSD / norm2d * d
	...
	1001 format(/,1X,'Solver: Newton-Gradient method with safeguards')
