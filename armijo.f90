...
!		 1: stp = stpmin   
!		-1: the initial trial step-size is less than stepmin
!		-2: the search direction is not a descent direction
!
	
	! LOCAL SCALARS
	integer :: i, nfevbt
	real(kind=8), parameter :: zero = 0.0d0
	real(kind=8) :: f,maxg0,ftest
	logical :: sdc
  ...
  ! Compute maxg0
	...
	if ( sdc ) then
		...
		call backtrackingMO(evalf,stp,stpmin,m,f0,g0,fend,ftest,nfevbt,info)
		nfev = nfev + nfevbt
	end if
...
subroutine backtrackingMO(evalf,stp,stpmin,m,f0,g0,fend,ftest,nfev,info)	
	...
	integer, intent(in) :: m
	integer, intent(out) :: nfev,info
	real(kind=8), intent(in) :: stpmin,ftest
	real(kind=8), intent(inout) :: stp
	!logical,intent(in) :: iprint
	...
	Main_loop: do
		...
		Inner_loop: do
			...
			if ( f <= f0(ind) + ftest * stp ) exit Inner_loop
